...
par(mar = c(4, 4, 1, 1))
plot(rep(0,ncol(alpha_wc))~colnames(alpha_wc),ylim=c(0,25),ylab='Max. recruits/spawner',type='n',bty='l',xlab='Brood cohort year',xaxt='n')
axis(side=1,at=seq(1950,2020,by=5))
...
abline(0,1)
for(i in 1:nrow(stock_info_filtered)){
  lines(c(exp(alpha[i]),exp(r.alpha[i]))~rep(exp(alpha[i]),2),col=p_cols2[i],data=lt_prod)
}
...
points(exp(r.alpha)~exp(alpha),data=lt_prod_chi,bg=adjustcolor(sp_cols[1],alpha.f=0.2),col=adjustcolor(sp_cols[1],alpha.f=0.8),pch=21)
points(exp(r.alpha)~exp(alpha),data=lt_prod_chu,bg=adjustcolor(sp_cols[2],alpha.f=0.2),col=adjustcolor(sp_cols[2],alpha.f=0.8),pch=21)
points(exp(r.alpha)~exp(alpha),data=lt_prod_coh,bg=adjustcolor(sp_cols[3],alpha.f=0.2),col=adjustcolor(sp_cols[3],alpha.f=0.8),pch=21)
points(exp(r.alpha)~exp(alpha),data=lt_prod_pi,bg=adjustcolor(sp_cols[4],alpha.f=0.2),col=adjustcolor(sp_cols[4],alpha.f=0.8),pch=21)
points(exp(r.alpha)~exp(alpha),data=lt_prod_soc,bg=adjustcolor(sp_cols[5],alpha.f=0.2),col=adjustcolor(sp_cols[5],alpha.f=0.8),pch=21)
...
par(mfrow=c(1,2))
plot(r.umsy~umsy,data=lt_prod,ylab='Recent (last 10y) Umsy',xlab='Long-term average Umsy',bty='l',type='n',xlim=c(0,1),ylim=c(0,1))
abline(0,1)
for(i in 1:nrow(stock_info_filtered)){
