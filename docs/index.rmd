...
sp_cols=c("darkgray", "darkgreen", "darkblue","redorange","darkred")
...
lat_lons= stock_info_filtered %>% group_by(lat,lon,species) %>% summarize(n=n(),.groups = 'keep')

ggplot(data = world) + 
  geom_sf(fill= 'antiquewhite') +  xlab("Longitude") + ylab("Latitude") +
  geom_point(data = lat_lons, mapping = aes(x = lon, y = lat,fill=species), color = species, size = 2.5*log(lat_lons$n+1), alpha = 0.7)+
scale_fill_manual(values=c("gray", "darkgreen", "navy","sienna","darkred"))

#TO DO: contours for ocean depths (low priority), labels for WC/GOA/BS
