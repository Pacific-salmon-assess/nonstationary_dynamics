...
prod_mat1=matrix(nrow=nrow(stock_info_filtered),ncol=max(stock_dat2$broodyear)-min(stock_dat2$broodyear)+1)
prod_mat2=matrix(nrow=nrow(stock_info_filtered),ncol=max(stock_dat2$broodyear)-min(stock_dat2$broodyear)+1)
prod_mat1rs=matrix(nrow=nrow(stock_info_filtered),ncol=max(stock_dat2$broodyear)-min(stock_dat2$broodyear)+1)
prod_mat2rs=matrix(nrow=nrow(stock_info_filtered),ncol=max(stock_dat2$broodyear)-min(stock_dat2$broodyear)+1)
colnames(prod_matr)=seq(min(stock_dat2$broodyear),max(stock_dat2$broodyear))
colnames(prod_mat1)=seq(min(stock_dat2$broodyear),max(stock_dat2$broodyear))
colnames(prod_mat2)=seq(min(stock_dat2$broodyear),max(stock_dat2$broodyear))
colnames(prod_mat1rs)=seq(min(stock_dat2$broodyear),max(stock_dat2$broodyear))
colnames(prod_mat2rs)=seq(min(stock_dat2$broodyear),max(stock_dat2$broodyear))
...
for(i in 1:nrow(stock_info_filtered)){
  ...
  prod_m=apply(exp(prod),2,median)
  prod_scale=prod/median(unlist(prod))
  prod_scale2=prod-median(unlist(prod))/sd(unlist(prod))
  prod_scale_rs=exp(prod)/median(unlist(exp(prod)))
  prod_scale2_rs=exp(prod)-median(unlist(exp(prod)))/sd(unlist(exp(prod)))
  m_prod_scale1=apply(prod_scale,2,median) #scaled as multiplier of median
  m_prod_scale2=apply(prod_scale2,2,median)
  m_prod_scale1rs=apply(prod_scale_rs,2,median) #scaled as multiplier of median
  m_prod_scale2rs=apply(prod_scale2_rs,2,median)
  
  prod_matr[i,match(seq(min(df$by),max(df$by)),colnames(prod_matr))]=prod_m
  prod_mat1[i,match(seq(min(df$by),max(df$by)),colnames(prod_mat1))]=m_prod_scale1
  prod_mat2[i,match(seq(min(df$by),max(df$by)),colnames(prod_mat2))]=m_prod_scale2
  prod_mat1rs[i,match(seq(min(df$by),max(df$by)),colnames(prod_mat1rs))]=m_prod_scale1rs
  prod_mat2rs[i,match(seq(min(df$by),max(df$by)),colnames(prod_mat2rs))]=m_prod_scale2rs
}
```


```{r, plot prod global prod trend}
plot(rep(0,ncol(prod_matr))~colnames(prod_matr),ylim=c(min(na.omit(as.vector(prod_matr))),25),ylab='Max. recruits/spawner',type='n',bty='l',xlab='Brood cohort year')
...
pi_alpha1=prod_matr[as.numeric(rownames(stock_info_filtered[stock_info_filtered$species=='Pink-Even',])),]
pi_alpha2=prod_matr[as.numeric(rownames(stock_info_filtered[stock_info_filtered$species=='Pink-odd',])),]
pi_alpha=rbind(pi_alpha1,pi_alpha2)
soc_alpha=prod_matr[as.numeric(rownames(stock_info_filtered[stock_info_filtered$species=='Sockeye',])),]




chi_alpha_sc=prod_mat2[as.numeric(rownames(stock_info_filtered[stock_info_filtered$species=='Chinook',])),]
chu_alpha_sc=prod_mat2[as.numeric(rownames(stock_info_filtered[stock_info_filtered$species=='Chum',])),]
coh_alpha_sc=prod_mat2[as.numeric(rownames(stock_info_filtered[stock_info_filtered$species=='Coho',])),]
pi_alpha_sc1=prod_mat2[as.numeric(rownames(stock_info_filtered[stock_info_filtered$species=='Pink-Even',])),]
pi_alpha_sc2=prod_mat2[as.numeric(rownames(stock_info_filtered[stock_info_filtered$species=='Pink-odd',])),]
pi_alpha_sc=rbind(pi_alpha_sc1,pi_alpha_sc2)
soc_alpha_sc=prod_mat2[as.numeric(rownames(stock_info_filtered[stock_info_filtered$species=='Sockeye',])),]
```
...
```{r, prod trends sp}
par(mar = c(4, 4, 1, 1))
plot(rep(0,ncol(chi_alpha))~colnames(chi_alpha),ylim=c(min(na.omit(as.vector(chi_alpha))),25),ylab='Max. recruits/spawner',type='n',bty='l',xlab='Brood cohort year')
for(i in 1:nrow(chi_alpha)){
  lines(chi_alpha[i,]~colnames(chi_alpha),col=adjustcolor('darkgray',alpha.f=0.3))
}
m.prod.5f=move_avg_mat(chi_alpha);m.prod.5f$y=colnames(chi_alpha)[1:c(ncol(chi_alpha)-2)]
m.prod.5f=m.prod.5f[complete.cases(m.prod.5f[,1]),]
x<- c(seq(m.prod.5f$y[1],m.prod.5f$y[nrow(m.prod.5f)]), rev(seq(m.prod.5f$y[1],m.prod.5f$y[nrow(m.prod.5f)])))
y<- c(m.prod.5f[,1]-2*m.prod.5f[,2], rev(m.prod.5f[,1]+2*m.prod.5f[,2]))
...
#broken?? why?
plot(rep(0,ncol(chi_alpha_sc))~colnames(chi_alpha_sc),ylim=c(-5,5),ylab='Max. recruits/spawner',type='n',bty='l',xlab='Brood cohort year')
for(i in 1:nrow(chi_alpha_sc)){
  lines(chi_alpha_sc[i,]~colnames(chi_alpha_sc),col=adjustcolor('darkgray',alpha.f=0.3))
}
m.prod.5f=move_avg_mat(chi_alpha_sc);m.prod.5f$y=colnames(chi_alpha_sc)[1:c(ncol(chi_alpha_sc)-2)]
m.prod.5f=m.prod.5f[complete.cases(m.prod.5f[,1]),]
x<- c(seq(m.prod.5f$y[1],m.prod.5f$y[nrow(m.prod.5f)]), rev(seq(m.prod.5f$y[1],m.prod.5f$y[nrow(m.prod.5f)])))
y<- c(m.prod.5f[,1]-2*m.prod.5f[,2], rev(m.prod.5f[,1]+2*m.prod.5f[,2]))
polygon(x, y, col = adjustcolor(sp_cols[1], alpha = 0.3), border=NA) # Add uncertainty polygon
...
```{r, prod trends sp}
plot(rep(0,ncol(chu_alpha))~colnames(chu_alpha),ylim=c(min(na.omit(as.vector(chu_alpha))),25),ylab='Max. recruits/spawner',type='n',bty='l',xlab='Brood cohort year')
for(i in 1:nrow(chu_alpha)){
  lines(chu_alpha[i,]~colnames(chu_alpha),col=adjustcolor('darkgray',alpha.f=0.3))
}
m.prod.5f=move_avg_mat(chu_alpha);m.prod.5f$y=colnames(chu_alpha)[1:c(ncol(chu_alpha)-2)]
m.prod.5f=m.prod.5f[complete.cases(m.prod.5f[,1]),]
x<- c(seq(m.prod.5f$y[1],m.prod.5f$y[nrow(m.prod.5f)]), rev(seq(m.prod.5f$y[1],m.prod.5f$y[nrow(m.prod.5f)])))
y<- c(m.prod.5f[,1]-2*m.prod.5f[,2], rev(m.prod.5f[,1]+2*m.prod.5f[,2]))
polygon(x, y, col = adjustcolor(sp_cols[2], alpha = 0.3), border=NA) # Add uncertainty polygon
...
```{r, prod trends sp}
plot(rep(0,ncol(coh_alpha))~colnames(coh_alpha),ylim=c(min(na.omit(as.vector(coh_alpha))),25),ylab='Max. recruits/spawner',type='n',bty='l',xlab='Brood cohort year')
for(i in 1:nrow(coh_alpha)){
  lines(coh_alpha[i,]~colnames(coh_alpha),col=adjustcolor('darkgray',alpha.f=0.3))
}
m.prod.5f=move_avg_mat(coh_alpha);m.prod.5f$y=colnames(coh_alpha)[1:c(ncol(coh_alpha)-2)]
m.prod.5f=m.prod.5f[complete.cases(m.prod.5f[,1]),]
x<- c(seq(m.prod.5f$y[1],m.prod.5f$y[nrow(m.prod.5f)]), rev(seq(m.prod.5f$y[1],m.prod.5f$y[nrow(m.prod.5f)])))
y<- c(m.prod.5f[,1]-2*m.prod.5f[,2], rev(m.prod.5f[,1]+2*m.prod.5f[,2]))
...
```{r, prod trends sp}
plot(rep(0,ncol(pi_alpha))~colnames(pi_alpha),ylim=c(min(na.omit(as.vector(pi_alpha))),25),ylab='Max. recruits/spawner',type='n',bty='l',xlab='Brood cohort year')
for(i in 1:nrow(pi_alpha)){
  lines(pi_alpha[i,]~colnames(pi_alpha),col=adjustcolor('darkgray',alpha.f=0.3))
}
m.prod.5f=move_avg_mat(pi_alpha);m.prod.5f$y=colnames(pi_alpha)[1:c(ncol(pi_alpha)-2)]
m.prod.5f=m.prod.5f[complete.cases(m.prod.5f[,1]),]
x<- c(seq(m.prod.5f$y[1],m.prod.5f$y[nrow(m.prod.5f)]), rev(seq(m.prod.5f$y[1],m.prod.5f$y[nrow(m.prod.5f)])))
y<- c(m.prod.5f[,1]-2*m.prod.5f[,2], rev(m.prod.5f[,1]+2*m.prod.5f[,2]))
...
```{r, prod trends sp}
plot(rep(0,ncol(soc_alpha))~colnames(soc_alpha),ylim=c(min(na.omit(as.vector(soc_alpha))),25),ylab='Max. recruits/spawner',type='n',bty='l',xlab='Brood cohort year')
for(i in 1:nrow(soc_alpha)){
  lines(soc_alpha[i,]~colnames(soc_alpha),col=adjustcolor('darkgray',alpha.f=0.3))
}
m.prod.5f=move_avg_mat(soc_alpha);m.prod.5f$y=colnames(soc_alpha)[1:c(ncol(soc_alpha)-2)]
m.prod.5f=m.prod.5f[complete.cases(m.prod.5f[,1]),]
x<- c(seq(m.prod.5f$y[1],m.prod.5f$y[nrow(m.prod.5f)]), rev(seq(m.prod.5f$y[1],m.prod.5f$y[nrow(m.prod.5f)])))
y<- c(m.prod.5f[,1]-2*m.prod.5f[,2], rev(m.prod.5f[,1]+2*m.prod.5f[,2]))
...
cap_matr=matrix(nrow=nrow(stock_info_filtered),ncol=max(stock_dat2$broodyear)-min(stock_dat2$broodyear)+1)
cap_mat1=matrix(nrow=nrow(stock_info_filtered),ncol=max(stock_dat2$broodyear)-min(stock_dat2$broodyear)+1)
cap_mat2=matrix(nrow=nrow(stock_info_filtered),ncol=max(stock_dat2$broodyear)-min(stock_dat2$broodyear)+1)
cap_mat1rs=matrix(nrow=nrow(stock_info_filtered),ncol=max(stock_dat2$broodyear)-min(stock_dat2$broodyear)+1)
cap_mat2rs=matrix(nrow=nrow(stock_info_filtered),ncol=max(stock_dat2$broodyear)-min(stock_dat2$broodyear)+1)
colnames(cap_matr)=seq(min(stock_dat2$broodyear),max(stock_dat2$broodyear))
colnames(cap_mat1)=seq(min(stock_dat2$broodyear),max(stock_dat2$broodyear))
colnames(cap_mat2)=seq(min(stock_dat2$broodyear),max(stock_dat2$broodyear))
colnames(cap_mat1rs)=seq(min(stock_dat2$broodyear),max(stock_dat2$broodyear))
colnames(cap_mat2rs)=seq(min(stock_dat2$broodyear),max(stock_dat2$broodyear))
...
for(i in 1:nrow(stock_info_filtered)){
  ...
  cap=pl4[[i]][,grepl('Smax',colnames(pl4[[i]]))]
  cap_m=apply(log10(cap),2,median)
  cap_scale=cap/median(unlist(cap))
  cap_scale2=cap-median(unlist(cap))/sd(unlist(cap))
  m_cap_scale1=apply(cap_scale,2,median) #scaled as multiplier of median
  m_cap_scale2=apply(cap_scale2,2,median)

  cap_matr[i,match(seq(min(df$by),max(df$by)),colnames(cap_matr))]=cap_m
  cap_mat1[i,match(seq(min(df$by),max(df$by)),colnames(cap_mat1))]=m_cap_scale1
  cap_mat2[i,match(seq(min(df$by),max(df$by)),colnames(cap_mat2))]=m_cap_scale2
}
```


```{r, plot prod global prod trend}
plot(rep(0,ncol(cap_matr))~colnames(cap_matr),ylim=c(0,10),ylab='log10(Smax)',type='n',bty='l',xlab='Brood cohort year')
for(i in 1:nrow(cap_matr)){
  lines(cap_matr[i,]~colnames(cap_matr),col=adjustcolor('darkgray',alpha.f=0.3))
}
m.cap.5f=move_avg_mat(cap_matr);m.cap.5f$y=colnames(cap_matr)[1:c(ncol(cap_matr)-2)]
m.cap.5f=m.cap.5f[complete.cases(m.cap.5f[,1]),]
x<- c(seq(m.cap.5f$y[1],m.cap.5f$y[nrow(m.cap.5f)]), rev(seq(m.cap.5f$y[1],m.cap.5f$y[nrow(m.cap.5f)])))
y<- c(m.cap.5f[,1]-2*m.cap.5f[,2], rev(m.cap.5f[,1]+2*m.cap.5f[,2]))
polygon(x, y, col = adjustcolor('navy', alpha = 0.3), border=NA) # Add uncertainty polygon
lines(m.cap.5f[,1]~seq(min(x),max(x)),lwd=2,col='navy')
```

```{r, prod trends sp}
#rescale alpha for each stock

chi_cap=cap_matr[as.numeric(rownames(stock_info_filtered[stock_info_filtered$species=='Chinook',])),]
chu_cap=cap_matr[as.numeric(rownames(stock_info_filtered[stock_info_filtered$species=='Chum',])),]
coh_cap=cap_matr[as.numeric(rownames(stock_info_filtered[stock_info_filtered$species=='Coho',])),]
pi_cap1=cap_matr[as.numeric(rownames(stock_info_filtered[stock_info_filtered$species=='Pink-Even',])),]
pi_cap2=cap_matr[as.numeric(rownames(stock_info_filtered[stock_info_filtered$species=='Pink-odd',])),]
pi_cap=rbind(pi_cap1,pi_cap2)
soc_cap=cap_matr[as.numeric(rownames(stock_info_filtered[stock_info_filtered$species=='Sockeye',])),]
```
...
```{r, prod trends sp}
par(mar = c(4, 4, 1, 1))
plot(rep(0,ncol(chi_cap))~colnames(chi_cap),ylim=c(0,10),ylab='log10(Smax)',type='n',bty='l',xlab='Brood cohort year')
for(i in 1:nrow(chi_cap)){
  lines(chi_cap[i,]~colnames(chi_cap),col=adjustcolor('darkgray',alpha.f=0.3))
}
m.prod.5f=move_avg_mat(chi_cap);m.prod.5f$y=colnames(chi_cap)[1:c(ncol(chi_cap)-2)]
m.prod.5f=m.prod.5f[complete.cases(m.prod.5f[,1]),]
x<- c(seq(m.prod.5f$y[1],m.prod.5f$y[nrow(m.prod.5f)]), rev(seq(m.prod.5f$y[1],m.prod.5f$y[nrow(m.prod.5f)])))
y<- c(m.prod.5f[,1]-2*m.prod.5f[,2], rev(m.prod.5f[,1]+2*m.prod.5f[,2]))
polygon(x, y, col = adjustcolor(sp_cols[1], alpha.f = 0.3), border=NA) # Add uncertainty polygon
lines(m.prod.5f[,1]~seq(min(x),max(x)),lwd=2,col=sp_cols[1])
```
...
```{r, prod trends sp}
plot(rep(0,ncol(chu_cap))~colnames(chu_cap),ylim=c(0,10),ylab='log10(Smax)',type='n',bty='l',xlab='Brood cohort year')
for(i in 1:nrow(chu_cap)){
  lines(chu_cap[i,]~colnames(chu_cap),col=adjustcolor('darkgray',alpha.f=0.3))
}
m.prod.5f=move_avg_mat(chu_cap);m.prod.5f$y=colnames(chu_cap)[1:c(ncol(chu_cap)-2)]
m.prod.5f=m.prod.5f[complete.cases(m.prod.5f[,1]),]
x<- c(seq(m.prod.5f$y[1],m.prod.5f$y[nrow(m.prod.5f)]), rev(seq(m.prod.5f$y[1],m.prod.5f$y[nrow(m.prod.5f)])))
y<- c(m.prod.5f[,1]-2*m.prod.5f[,2], rev(m.prod.5f[,1]+2*m.prod.5f[,2]))
polygon(x, y, col = adjustcolor(sp_cols[2], alpha.f = 0.3), border=NA) # Add uncertainty polygon
lines(m.prod.5f[,1]~seq(min(x),max(x)),lwd=2,col=sp_cols[2])
```
...
```{r, prod trends sp}
plot(rep(0,ncol(coh_cap))~colnames(coh_cap),ylim=c(0,10),ylab='log10(Smax)',type='n',bty='l',xlab='Brood cohort year')
for(i in 1:nrow(coh_cap)){
  lines(coh_cap[i,]~colnames(coh_cap),col=adjustcolor('darkgray',alpha.f=0.3))
}
m.prod.5f=move_avg_mat(coh_cap);m.prod.5f$y=colnames(coh_cap)[1:c(ncol(coh_cap)-2)]
m.prod.5f=m.prod.5f[complete.cases(m.prod.5f[,1]),]
x<- c(seq(m.prod.5f$y[1],m.prod.5f$y[nrow(m.prod.5f)]), rev(seq(m.prod.5f$y[1],m.prod.5f$y[nrow(m.prod.5f)])))
y<- c(m.prod.5f[,1]-2*m.prod.5f[,2], rev(m.prod.5f[,1]+2*m.prod.5f[,2]))
polygon(x, y, col = adjustcolor(sp_cols[3], alpha.f = 0.3), border=NA) # Add uncertainty polygon
lines(m.prod.5f[,1]~seq(min(x),max(x)),lwd=2,col=sp_cols[3])
```
...
```{r, prod trends sp}
plot(rep(0,ncol(pi_cap))~colnames(pi_cap),ylim=c(0,10),ylab='log10(Smax)',type='n',bty='l',xlab='Brood piort year')
for(i in 1:nrow(pi_cap)){
  lines(pi_cap[i,]~colnames(pi_cap),col=adjustcolor('darkgray',alpha.f=0.3))
}
m.prod.5f=move_avg_mat(pi_cap);m.prod.5f$y=colnames(pi_cap)[1:c(ncol(pi_cap)-2)]
m.prod.5f=m.prod.5f[complete.cases(m.prod.5f[,1]),]
x<- c(seq(m.prod.5f$y[1],m.prod.5f$y[nrow(m.prod.5f)]), rev(seq(m.prod.5f$y[1],m.prod.5f$y[nrow(m.prod.5f)])))
y<- c(m.prod.5f[,1]-2*m.prod.5f[,2], rev(m.prod.5f[,1]+2*m.prod.5f[,2]))
polygon(x, y, col = adjustcolor(sp_cols[4], alpha.f = 0.3), border=NA) # Add uncertainty polygon
lines(m.prod.5f[,1]~seq(min(x),max(x)),lwd=2,col=sp_cols[4])
```
...
```{r, prod trends sp}
plot(rep(0,ncol(soc_cap))~colnames(soc_cap),ylim=c(0,10),ylab='log10(Smax)',type='n',bty='l',xlab='Brood piort year')
for(i in 1:nrow(soc_cap)){
  lines(soc_cap[i,]~colnames(soc_cap),col=adjustcolor('darkgray',alpha.f=0.3))
}
m.prod.5f=move_avg_mat(soc_cap);m.prod.5f$y=colnames(soc_cap)[1:c(ncol(soc_cap)-2)]
m.prod.5f=m.prod.5f[complete.cases(m.prod.5f[,1]),]
x<- c(seq(m.prod.5f$y[1],m.prod.5f$y[nrow(m.prod.5f)]), rev(seq(m.prod.5f$y[1],m.prod.5f$y[nrow(m.prod.5f)])))
y<- c(m.prod.5f[,1]-2*m.prod.5f[,2], rev(m.prod.5f[,1]+2*m.prod.5f[,2]))
polygon(x, y, col = adjustcolor(sp_cols[5], alpha.f = 0.3), border=NA) # Add uncertainty polygon
lines(m.prod.5f[,1]~seq(min(x),max(x)),lwd=2,col=sp_cols[5])
```
