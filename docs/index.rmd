...
par(mfrow=c(1,1))
par(mar=c(4,5,1,1))
hist(as.vector(d_rho),col=adjustcolor('gray',alpha=0.4),xlim=c(-1,1),border=adjustcolor('white',alpha=0.4),breaks=50,freq=T,main='',xlab=expression(paste(rho)),yaxt='n',ylab='')

par(new=T)
hist(m_rho,col=adjustcolor('black',alpha=0.4),xlim=c(-1,1),border=adjustcolor('white',alpha=0.2),breaks=30,freq=T,ylim=c(0,100),main='',xlab=expression(paste(rho)),xaxt='n',ylab='')
ticksat <- seq(0,2.5,by=0.1)
axis(1, ticksat, col="black", labels=NA,
     tcl=-0.2, lwd=0, lwd.ticks=1)
```
...
x_new=seq(0,max(df$S),length.out=1000)
p1=exp(median(pl1[[i]][,grepl('log_a',colnames(pl1[[i]]))])-median(pl1[[i]][,grepl('b',colnames(pl1[[i]]))])*x_new)*x_new
...
pr1=median(pl1[[i]][,grepl('log_a',colnames(pl1[[i]]))])-median(pl1[[i]][,grepl('b',colnames(pl1[[i]]))])*df$S
...
lines(R~S,data=df)
for(t in 1:nrow(df)){
  p=exp(median(pl4[[i]][,grepl('log_a',colnames(pl4[[i]]))])-median(1/pl4[[i]][,grepl('Smax',colnames(pl4[[i]]))][,t])*x_new)*x_new
  lines(p~x_new,lwd=2,col=col.p[t])
}
points(R~S,data=df,pch=21,cex=1.5,bg=col.p)


plot(apply(pl4[[i]][,grepl('Smax',colnames(pl4[[i]]))],2,median)~seq(min(df$by),max(df$by)),data=df,bty='l',pch=21,cex=1.5,bg=col.p,ylab='Smax',xlab='year',ylim=c(0,quantile(unlist(pl4[[i]][,grepl('Smax',colnames(pl4[[i]]))]),0.95)))
lci=apply(pl4[[i]][,grepl('Smax',colnames(pl4[[i]]))],2,quantile,0.05)
uci=apply(pl4[[i]][,grepl('Smax',colnames(pl4[[i]]))],2,quantile,0.95)
x<- c(seq(min(df$by),max(df$by)), rev(seq(min(df$by),max(df$by))))
y<- c(lci, rev(uci))
polygon(x, y, col = adjustcolor('gray', alpha = 0.2), border=NA) # Add uncertainty polygon
lines(apply(pl4[[i]][,grepl('Smax',colnames(pl4[[i]]))],2,median)~seq(min(df$by),max(df$by)),lwd=1)
...
col.p=cols[cut(1-apply(pl5[[i]][,grepl('gamma',colnames(pl5[[i]]))][,1:nrow(df)],2,median), breaks = seq(0,1,by=0.1), labels = FALSE)]
lines(R~S,data=df)
p=exp(median(pl5[[i]][,grepl('log_a',colnames(pl5[[i]]))][,1])-median(pl5[[i]][,grepl('b',colnames(pl5[[i]]))])*x_new)*x_new
lines(p~x_new,lwd=2,col=cols[1])
p=exp(median(pl5[[i]][,grepl('log_a',colnames(pl5[[i]]))][,2])-median(pl5[[i]][,grepl('b',colnames(pl5[[i]]))])*x_new)*x_new
lines(p~x_new,lwd=2,col=cols[length(cols)])
points(R~S,data=df,pch=21,cex=1.5,bg=col.p)


plot(1-apply(pl5[[i]][,grepl('gamma',colnames(pl5[[i]]))][,1:nrow(df)],2,median)~df$by,data=df,bty='l',pch=21,cex=1.5,bg='black',ylab='p(high prod. regime)',xlab='year',ylim=c(0,1),type='n')
lci=1-apply(pl5[[i]][,grepl('gamma',colnames(pl5[[i]]))][,1:nrow(df)],2,quantile,0.05)
uci=1-apply(pl5[[i]][,grepl('gamma',colnames(pl5[[i]]))][,1:nrow(df)],2,quantile,0.95)
x<- c(df$by, rev(df$by))
y<- c(lci, rev(uci))
polygon(x, y, col = adjustcolor('gray', alpha = 0.2), border=NA) # Add uncertainty polygon
lines(1-apply(pl5[[i]][,grepl('gamma',colnames(pl5[[i]]))][,1:nrow(df)],2,median)~df$by,lwd=1)
points(1-apply(pl5[[i]][,grepl('gamma',colnames(pl5[[i]]))][,1:nrow(df)],2,median)~df$by,pch=21,cex=1.5,bg=col.p)

#smax regimes
# par(mfrow=c(1,2))
plot(R~S,data=df,bty='l',pch=21,cex=1.5,type='n',main='regime cap.',xlim=c(0,max(df$S)),ylim=c(0,max(df$R)))
cols=RColorBrewer::brewer.pal(10,'BrBG')
col.p=rev(cols)[cut(1-apply(pl6[[i]][,grepl('gamma',colnames(pl6[[i]]))][,1:nrow(df)],2,median), breaks = seq(0,1,by=0.1), labels = FALSE)]
lines(R~S,data=df)
p=exp(median(pl6[[i]][,grepl('log_a',colnames(pl6[[i]]))])-median(pl6[[i]][,grepl('b',colnames(pl6[[i]]))][,1])*x_new)*x_new
lines(p~x_new,lwd=2,col=cols[length(cols)])
p=exp(median(pl6[[i]][,grepl('log_a',colnames(pl6[[i]]))])-median(pl6[[i]][,grepl('b',colnames(pl6[[i]]))][,2])*x_new)*x_new
lines(p~x_new,lwd=2,col=cols[1])
points(R~S,data=df,pch=21,cex=1.5,bg=col.p)

plot(apply(pl6[[i]][,grepl('gamma',colnames(pl6[[i]]))][,1:nrow(df)],2,median)~df$by,data=df,bty='l',pch=21,cex=1.5,bg='black',ylab='p(high cap. regime)',xlab='year',ylim=c(0,1),type='n')
lci=apply(pl6[[i]][,grepl('gamma',colnames(pl6[[i]]))][,1:nrow(df)],2,quantile,0.05)
uci=apply(pl6[[i]][,grepl('gamma',colnames(pl6[[i]]))][,1:nrow(df)],2,quantile,0.95)
x<- c(df$by, rev(df$by))
y<- c(lci, rev(uci))
polygon(x, y, col = adjustcolor('gray', alpha = 0.2), border=NA) # Add uncertainty polygon
lines(apply(pl6[[i]][,grepl('gamma',colnames(pl6[[i]]))][,1:nrow(df)],2,median)~df$by,lwd=1)
points(apply(pl6[[i]][,grepl('gamma',colnames(pl6[[i]]))][,1:nrow(df)],2,median)~df$by,pch=21,cex=1.5,bg=col.p)
...
col.p=cols[cut(1-apply(pl7[[i]][,grepl('gamma',colnames(pl7[[i]]))][,1:nrow(df)],2,median), breaks = seq(0,1,by=0.1), labels = FALSE)]
lines(R~S,data=df)
p=exp(median(pl7[[i]][,grepl('log_a',colnames(pl7[[i]]))][,1])-median(pl7[[i]][,grepl('b',colnames(pl7[[i]]))][,1])*x_new)*x_new
lines(p~x_new,lwd=2,col=cols[1])
p=exp(median(pl7[[i]][,grepl('log_a',colnames(pl7[[i]]))][,2])-median(pl7[[i]][,grepl('b',colnames(pl7[[i]]))][,2])*x_new)*x_new
lines(p~x_new,lwd=2,col=cols[length(cols)])
points(R~S,data=df,pch=21,cex=1.5,bg=col.p)

plot(1-apply(pl7[[i]][,grepl('gamma',colnames(pl7[[i]]))][,1:nrow(df)],2,median)~df$by,data=df,bty='l',pch=21,cex=1.5,bg='black',ylab='p(high prod. regime)',xlab='year',ylim=c(0,1),type='n')
lci=1-apply(pl7[[i]][,grepl('gamma',colnames(pl7[[i]]))][,1:nrow(df)],2,quantile,0.05)
uci=1-apply(pl7[[i]][,grepl('gamma',colnames(pl7[[i]]))][,1:nrow(df)],2,quantile,0.95)
x<- c(df$by, rev(df$by))
y<- c(lci, rev(uci))
polygon(x, y, col = adjustcolor('gray', alpha = 0.2), border=NA) # Add uncertainty polygon
lines(1-apply(pl7[[i]][,grepl('gamma',colnames(pl7[[i]]))][,1:nrow(df)],2,median)~df$by,lwd=1)
points(1-apply(pl7[[i]][,grepl('gamma',colnames(pl7[[i]]))][,1:nrow(df)],2,median)~df$by,pch=21,cex=1.5,bg=col.p)
