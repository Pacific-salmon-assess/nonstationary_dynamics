...
scale_fill_manual(values=c("darkgray", "darkgreen", "navy","darkorange","darkred"))+ 
...
#Model selection critera

For each Pacific salmon spawner-recruit time-series we estimate each of these 8 models using either Stan or TMB. We compared the likelihoods of observing the data given each of these models, which are then used to estimate different model selection criteria.

Broadly the model selection criteria fell into two categories: 'in-sample' likelihoods of the model fits and 'out-of-sample' likelihoods, where a portion of the dataset is omitted from the model fitting stage and likelihoods estimate out-of-sample prediction accuracy. We will examine both of these
