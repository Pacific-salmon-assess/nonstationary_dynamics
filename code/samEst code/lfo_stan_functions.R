...
stan_refit<- function(sm,newdata,oos,regime=FALSE,K=NULL){
  ...
  if(regime==FALSE){
    r = rstan::sampling(sm, 
                        ...
                        control = list(adapt_delta = 0.95,max_treedepth=15), warmup = 200, chains = 6, iter = 700)
  }
  if(regime==TRUE){
    r = rstan::sampling(sm, 
                        data = list(N=nrow(newdata),
...
#' @examples
#' r=stan_refit(sm=mod3,newdata=df,oos=12)
stan_lfo_cv2=function(mod,type=c('static','tv','regime'),df,L=10,K=NULL){
  #mod = model to fit (model name for cmdstanr)
  #tv = 0 for static model; 1 for time-varying (for calculating elpds)
  #df = full data frame
  #L = starting point for LFO-CV (default 10)
  ...
  if(type=='tv'){
   return(list(loglik_exact_1b,loglik_exact_3b,loglik_exact_5b))
  }
  if(type=='regime'){
    return(list(loglik_exact_1b,loglik_exact_3b,loglik_exact_5b))
  }
...
model_weights<- function(x,type=c('full','d90','d80')){
  ...
  if(type=='d90'){
    pp90=apply(x,2,mean) #
    d90=quantile(pp90,0.1)
    x=x[,which(pp90>d90)]
    elpd_1=apply(x,1,sum) #
  }
  if(type=='d80'){
    pp80=apply(x,2,mean) #
    d80=quantile(pp80,0.2)
    x=x[,which(pp80>d80)]
    elpd_1=apply(x,1,sum) #
  }
...
#' @examples
#' model_weights(rbind(ll1,ll2))
model_weights2<- function(x,type=c('full','d90','d80')){
  #x = dataframe of pointwise log likelihoods
  if(type=='full'){
    elpd_1=apply(x,1,sum) #
  }
