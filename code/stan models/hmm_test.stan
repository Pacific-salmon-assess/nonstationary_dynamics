...
data {
 int<lower=1> T;//number of annual samples (time-series length)
  vector[T] R_S; //log(recruits per spawner)
  vector[T] S; //spawners in time T
...
ordered[K] a_max; // max. productivity
...
logalpha[1] = log(pi1) + normal_lpdf(R_S[1] |a_max - b*S[1], sigma);
for (t in 2:T) {
for (j in 1:K) { // j = current (t)
for (i in 1:K) { // i = previous (t-1)
// Murphy (2012) p. 609 eq. 17.48
// belief state + transition prob + local evidence at t
accumulator[i] = logalpha[t-1, i] + log(A[i, j]) + normal_lpdf(R_S[t] | a_max[j] - b*S[t], sigma[j]);
}
...
accumulator[i] = logbeta[t, i] + log(A[j, i]) + normal_lpdf(R_S[t] | a_max[i] - b*S[t], sigma[i]);
}
...
for (j in 1:K)
delta[1, K] = normal_lpdf(R_S[1] | a_max[j] - b*S[1], sigma[j]);
for (t in 2:T) {
for (j in 1:K) { // j = current (t)
delta[t, j] = negative_infinity();
for (i in 1:K) { // i = previous (t-1)
real logp;
logp = delta[t-1, i] + log(A[i, j]) + normal_lpdf(R_S[t] | a_max[j] - b*S[t], sigma[j]);
if (logp > delta[t, j]) {
bpointer[t, j] = i;
delta[t, j] = logp;
}
