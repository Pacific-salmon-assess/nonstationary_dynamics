...
plot(apply(d$log_a_g,2,median),ylim=c(-1,4),type='l',bty='l',ylab='Productivity - log alpha',lwd=2)
for(i in 1:13){
...
test3=rstan::stan_model(model_code=mc3)
...
plot(prod_avg~colnames(prod_mat),ylim=c(-1,4),type='n',bty='l',ylab='Productivity - log alpha',lwd=2)
for(i in 1:16){
  lines(apply(d$log_a_t[,,i],2,median)~seq(min(sk_nass_stocks$begin),max(sk_nass_stocks$end)),col=adjustcolor('black',alpha.f=0.2))
